...
#pragma omp parallel for
  for (size_t i = 0; i < num; i++) {
    if (i == 0) {
      ...
      ret[i] = std::string(data + positions[i - 1] + 1,
                           positions[i] - positions[i - 1] - 1);
    }
  }
  if (positions.back() != size - 1)
    ret.emplace_back(
        std::string(data + positions.back() + 1, size - positions.back()));
...
void importPerson(snb::PersonSchema &personSchema, std::string path,
  ...
  assert(all_persons[0] == "id|firstName|lastName|gender|birthday|creationDate|"
                           "locationIP|browserUsed|place|language|email");

  std::vector<uint64_t> person_vids(all_persons.size()),
      place_vids(all_persons.size());
#pragma omp parallel for
  ...
  for (size_t i = 1; i < all_persons.size(); i++) {
    ...
    auto person_buf = snb::PersonSchema::createPerson(
        std::stoull(person_v[0]), person_v[1], person_v[2], person_v[3],
        std::chrono::duration_cast<std::chrono::hours>(
            birthday.time_since_epoch())
            .count(),
        emails_v, speaks_v, person_v[7], person_v[6],
        std::chrono::duration_cast<std::chrono::milliseconds>(
            creationDate.time_since_epoch())
            .count(),
        place_vid);

    personSchema.insertName(person_v[1] + snb::key_spacing + person_v[0],
                            person_vid);

    loader.put_vertex(person_vid,
                      std::string_view(person_buf.data(), person_buf.size()));
    loader.put_edge(place_vid, (label_t)snb::EdgeSchema::Place2Person,
                    person_vid, std::string_view(), true);

    //        const snb::PersonSchema::Person *person = (const
    //        snb::PersonSchema::Person*)person_buf.data(); std::cout <<
    //        person->id << "|" << person->place << "|" <<
    ...
    //            << std::string(person->browserUsed(),
    //            person->browserUsedLen()) << "|"
    //            << std::string(person->locationIP(), person->locationIPLen())
    //            << "|";
    //        std::time_t t =
    //        std::chrono::system_clock::to_time_t(std::chrono::system_clock::time_point(std::chrono::hours(person->birthday)));
    //        std::cout << std::ctime(&t) << "|";
    //        t =
    //        std::chrono::system_clock::to_time_t(std::chrono::system_clock::time_point(std::chrono::milliseconds(person->creationDate)));
    //        std::cout << std::ctime(&t) << "|";
    //        emails_v = split(std::string(person->emails(),
    //        person->emailsLen()), zero_split); for(auto e:emails_v) std::cout
    //        << e << ","; std::cout << "|"; speaks_v =
    //        split(std::string(person->speaks(), person->speaksLen()),
    //        zero_split); for(auto e:speaks_v) std::cout << e << ","; std::cout
    //        << std::endl;
  }
}
...
#pragma omp parallel for
  for (size_t i = 1; i < all_places.size(); i++) {
    ...
    isPartOf_vids[i] = place_v.size() > 4
                           ? placeSchema.findId(std::stoull(place_v[4]))
                           : (uint64_t)-1;
  }
  ...
  for (size_t i = 1; i < all_places.size(); i++) {
    ...
    auto place_buf = snb::PlaceSchema::createPlace(
        std::stoull(place_v[0]), place_v[1], place_v[2], type, isPartOf_vid);

    loader.put_vertex(place_vid,
                      std::string_view(place_buf.data(), place_buf.size()));
    if (isPartOf_vid != (uint64_t)-1) {
      loader.put_edge(isPartOf_vid, (label_t)snb::EdgeSchema::Place2Place_down,
                      place_vid, std::string_view(), true);
    }

    //        const snb::PlaceSchema::Place *place = (const
    //        snb::PlaceSchema::Place*)place_buf.data(); std::cout << place->id
    //        << "|" << std::string(place->name(), place->nameLen()) << "|"
    //            << std::string(place->url(), place->urlLen()) << "|"  <<
    //            (int)place->type << "|" << place->isPartOf << std::endl;
  }
}
...
#pragma omp parallel for
  ...
  for (size_t i = 1; i < all_orgs.size(); i++) {
    ...
    auto org_buf = snb::OrgSchema::createOrg(std::stoull(org_v[0]), type,
                                             org_v[2], org_v[3], place_vid);

    loader.put_vertex(org_vid,
                      std::string_view(org_buf.data(), org_buf.size()));
    loader.put_edge(place_vid, (label_t)snb::EdgeSchema::Place2Org, org_vid,
                    std::string_view(), true);

    //        const snb::OrgSchema::Org *org = (const
    //        snb::OrgSchema::Org*)org_buf.data(); std::cout << org->id << "|"
    //        << std::string(org->name(), org->nameLen()) << "|"
    //            << std::string(org->url(), org->urlLen()) << "|"  <<
    //            (int)org->type << "|" << org->place << std::endl;
  }
}

void importPost(snb::MessageSchema &postSchema, std::string path,
                snb::PersonSchema &personSchema, snb::ForumSchema &forumSchema,
                snb::PlaceSchema &placeSchema) {
  auto all_messages = parallel_readlines(path + snb::postPathSuffix);
  assert(all_messages[0] == "id|imageFile|creationDate|locationIP|browserUsed|"
  ...
  all_messages.clear();
  tbb::parallel_sort(
      message_vs.begin(), message_vs.end(),
      [](const std::vector<std::string> &a, const std::vector<std::string> &b) {
        return a[2] < b[2];
      });

  std::vector<uint64_t> message_vids(message_vs.size()),
      creator_vids(message_vs.size()), forumid_vids(message_vs.size()),
...
#pragma omp parallel for
  ...
  for (size_t i = 0; i < message_vs.size(); i++) {
    auto &message_v = message_vs[i];
    snb::MessageSchema::Message::Type type =
        snb::MessageSchema::Message::Type::Post;
    assert(std::stoull(message_v[7]) <= message_v[6].length());
    std::chrono::system_clock::time_point creationDate =
        from_time(message_v[2]);

    uint64_t message_vid = message_vids[i];
    uint64_t creator_vid = creator_vids[i];
    uint64_t forumid_vid = forumid_vids[i];
    uint64_t place_vid = place_vids[i];
    uint64_t creationDateInt =
        std::chrono::duration_cast<std::chrono::milliseconds>(
            creationDate.time_since_epoch())
            .count();

    auto message_buf = snb::MessageSchema::createMessage(
        std::stoull(message_v[0]), message_v[1], creationDateInt, message_v[3],
        message_v[4], message_v[5], message_v[6], creator_vid, forumid_vid,
        place_vid, (uint64_t)-1, (uint64_t)-1, type);

    loader.put_vertex(message_vid,
                      std::string_view(message_buf.data(), message_buf.size()));
    loader.put_edge(forumid_vid, (label_t)snb::EdgeSchema::Forum2Post,
                    message_vid, std::string_view(), true);
    loader.put_edge(
        creator_vid, (label_t)snb::EdgeSchema::Person2Post_creator, message_vid,
        std::string_view((char *)&creationDateInt, sizeof(creationDateInt)),
        true);

    //        const snb::MessageSchema::Message *message = (const
    //        snb::MessageSchema::Message*)message_buf.data(); std::cout <<
    //        message->id << "|" << std::string(message->imageFile(),
    //        message->imageFileLen()) << "|"
    //            << std::string(message->locationIP(),
    //            message->locationIPLen()) << "|"
    //            << std::string(message->browserUsed(),
    //            message->browserUsedLen()) << "|"
    //            << std::string(message->language(), message->languageLen()) <<
    //            "|"
    //            << std::string(message->content(), message->contentLen()) <<
    //            "|"
    //            << message->creator << "|" << message->forumid << "|" <<
    //            message->place << "|"
    //            << message->replyOfPost << "|" << message->replyOfComment <<
    //            "|" << (int)message->type << "|";
    //        std::time_t t =
...
void importComment(snb::MessageSchema &commentSchema, std::string path,
  ...
  assert(all_messages[0] == "id|creationDate|locationIP|browserUsed|content|"
                            "length|creator|place|replyOfPost|replyOfComment");
  std::vector<std::vector<std::string>> message_vs;
  ...
  std::vector<uint64_t> message_vids(message_vs.size()),
      creator_vids(message_vs.size()), place_vids(message_vs.size());
  std::vector<uint64_t> replyOfPost_vids(message_vs.size()),
      replyOfComment_vids(message_vs.size());
#pragma omp parallel for
  for (size_t i = 0; i < message_vs.size(); i++) {
    auto &message_v = message_vs[i];
    ...
    replyOfPost_vids[i] = !message_v[8].empty()
                              ? postSchema.findId(std::stoull(message_v[8]))
                              : (uint64_t)-1;
    replyOfComment_vids[i] =
        message_v.size() > 9 ? commentSchema.findId(std::stoull(message_v[9]))
                             : (uint64_t)-1;
  }
  auto loader = graph->begin_batch_loader();
  ...
  for (size_t i = 0; i < message_vs.size(); i++) {
    ...
    auto message_buf = snb::MessageSchema::createMessage(
        std::stoull(message_v[0]), std::string(), creationDateInt, message_v[2],
        message_v[3], std::string(), message_v[4], creator_vid, (uint64_t)-1,
        place_vid, replyOfPost_vid, replyOfComment_vid, type);

    loader.put_vertex(message_vid,
                      std::string_view(message_buf.data(), message_buf.size()));
    loader.put_edge(
        creator_vid, (label_t)snb::EdgeSchema::Person2Comment_creator,
        message_vid,
        std::string_view((char *)&creationDateInt, sizeof(creationDateInt)),
        true);
    if (replyOfPost_vid != (uint64_t)-1) {
      loader.put_edge(
          replyOfPost_vid, (label_t)snb::EdgeSchema::Message2Message_down,
          message_vid,
          std::string_view((char *)&creationDateInt, sizeof(creationDateInt)),
          true);
    }
    if (replyOfComment_vid != (uint64_t)-1) {
      loader.put_edge(
          replyOfComment_vid, (label_t)snb::EdgeSchema::Message2Message_down,
          message_vid,
          std::string_view((char *)&creationDateInt, sizeof(creationDateInt)),
          true);
    }
...
#pragma omp parallel for
  ...
  for (size_t i = 1; i < all_tags.size(); i++) {
    ...
    auto tag_buf = snb::TagSchema::createTag(std::stoull(tag_v[0]), tag_v[1],
                                             tag_v[2], hasType_vid);

    loader.put_vertex(tag_vid,
                      std::string_view(tag_buf.data(), tag_buf.size()));
    loader.put_edge(hasType_vid, (label_t)snb::EdgeSchema::TagClass2Tag,
                    tag_vid, std::string_view(), true);

    //        const snb::TagSchema::Tag *tag = (const
    //        snb::TagSchema::Tag*)tag_buf.data(); std::cout << tag->id << "|"
    //        << std::string(tag->name(), tag->nameLen()) << "|"
    //            << std::string(tag->url(), tag->urlLen()) << "|"  <<
    //            tag->hasType << std::endl;
  }
}
...
void importTagClass(snb::TagClassSchema &tagclassSchema, std::string path) {
  ...
  std::vector<uint64_t> tagclass_vids(all_tagclasss.size()),
      isSubclassOf_vids(all_tagclasss.size());
#pragma omp parallel for
  for (size_t i = 1; i < all_tagclasss.size(); i++) {
    ...
    isSubclassOf_vids[i] =
        tagclass_v.size() > 3
            ? tagclassSchema.findId(std::stoull(tagclass_v[3]))
            : (uint64_t)-1;
  }
  ...
  for (size_t i = 1; i < all_tagclasss.size(); i++) {
    ...
    auto tagclass_buf = snb::TagClassSchema::createTagClass(
        std::stoull(tagclass_v[0]), tagclass_v[1], tagclass_v[2],
        isSubclassOf_vid);

    loader.put_vertex(tagclass_vid, std::string_view(tagclass_buf.data(),
                                                     tagclass_buf.size()));
    if (isSubclassOf_vid != (uint64_t)-1) {
      loader.put_edge(isSubclassOf_vid,
                      (label_t)snb::EdgeSchema::TagClass2TagClass_down,
                      tagclass_vid, std::string_view(), true);
    }

    //        const snb::TagClassSchema::TagClass *tagclass = (const
    //        snb::TagClassSchema::TagClass*)tagclass_buf.data(); std::cout <<
    //        tagclass->id << "|" << std::string(tagclass->name(),
    //        tagclass->nameLen()) << "|"
    //            << std::string(tagclass->url(), tagclass->urlLen()) << "|"  <<
    //            tagclass->isSubclassOf << std::endl;
  }
}
...
void importForum(snb::ForumSchema &forumSchema, std::string path,
  ...
  std::vector<uint64_t> forum_vids(all_forums.size()),
      moderator_vids(all_forums.size());
#pragma omp parallel for
  ...
  for (size_t i = 1; i < all_forums.size(); i++) {
    ...
    auto forum_buf = snb::ForumSchema::createForum(
        std::stoull(forum_v[0]), forum_v[1],
        std::chrono::duration_cast<std::chrono::milliseconds>(
            creationDate.time_since_epoch())
            .count(),
        moderator_vid);

    loader.put_vertex(forum_vid,
                      std::string_view(forum_buf.data(), forum_buf.size()));
    loader.put_edge(moderator_vid,
                    (label_t)snb::EdgeSchema::Person2Forum_moderator, forum_vid,
                    std::string_view(), true);

    //        const snb::ForumSchema::Forum *forum = (const
    //        snb::ForumSchema::Forum*)forum_buf.data(); std::cout << forum->id
    //        << "|" << std::string(forum->title(), forum->titleLen()) << "|"
    //            << forum->moderator << "|";
    //        std::time_t t =
    //        std::chrono::system_clock::to_time_t(std::chrono::system_clock::time_point(std::chrono::milliseconds(forum->creationDate)));
    //        std::cout << std::ctime(&t);
  }
}

void importRawEdge(snb::Schema &xSchema, snb::Schema &ySchema,
                   snb::EdgeSchema x2y, snb::EdgeSchema y2x, std::string path) {
  auto all_rawEdges = parallel_readlines(path);

  std::vector<uint64_t> x_vids(all_rawEdges.size()),
      y_vids(all_rawEdges.size());
#pragma omp parallel for
...
void importDataEdge(snb::Schema &xSchema, snb::Schema &ySchema,
                    snb::EdgeSchema x2y, snb::EdgeSchema y2x, std::string path,
                    std::function<snb::Buffer(std::string)> dataParser,
                    bool sort) {
  ...
  if (sort)
    tbb::parallel_sort(
        dataEdge_vs.begin(), dataEdge_vs.end(),
        [](const std::vector<std::string> &a,
           const std::vector<std::string> &b) { return a[2] < b[2]; });
...
#pragma omp parallel for
  ...
  for (size_t i = 0; i < dataEdge_vs.size(); i++) {
    ...
    loader.put_edge(x_vid, (label_t)x2y, y_vid,
                    std::string_view(dataEdge_buf.data(), dataEdge_buf.size()),
                    true);
    loader.put_edge(y_vid, (label_t)y2x, x_vid,
                    std::string_view(dataEdge_buf.data(), dataEdge_buf.size()),
                    true);
  }
}
...
void importMember(std::string path) {
  ...
  std::vector<uint64_t> forum_ids(dataEdge_vs.size()),
      person_ids(dataEdge_vs.size());
#pragma omp parallel for
  for (size_t i = 0; i < dataEdge_vs.size(); i++) {
    auto &dataEdge_v = dataEdge_vs[i];
...
#pragma omp parallel for
  for (size_t i = 0; i < dataEdge_vs.size(); i++) {
    ...
    auto nbrs = engine.get_edges(person_id,
  ...
  for (size_t i = 0; i < dataEdge_vs.size(); i++) {
    ...
    *(uint64_t *)(buf.data() + sizeof(uint64_t)) = dataEdge_posts[i];
    ...
    loader.put_edge(forum_id, (label_t)snb::EdgeSchema::Forum2Person_member,
                    person_id, std::string_view(buf.data(), buf.size()), true);
    loader.put_edge(person_id, (label_t)snb::EdgeSchema::Person2Forum_member,
                    forum_id, std::string_view(buf.data(), buf.size()), true);
  }
}
...
#pragma omp parallel for
    ...
    double weight = 0;
    {
      ...
      while (nbrs.valid()) {
        ...
        auto rvid = comment->replyOfPost == (uint64_t)-1
                        ...
                        : comment->replyOfPost;
        auto reply =
            (snb::MessageSchema::Message *)engine.get_vertex(rvid).data();
  ...
  for (size_t i = 0; i < dataEdge_vs.size(); i++) {
    ...
    *(double *)(buf.data() + sizeof(uint64_t)) = dataEdge_weight[i];
    ...
    loader.put_edge(x_vid, (label_t)snb::EdgeSchema::Person2Person, y_vid,
                    std::string_view(buf.data(), buf.size()), true);
    loader.put_edge(y_vid, (label_t)snb::EdgeSchema::Person2Person, x_vid,
                    std::string_view(buf.data(), buf.size()), true);
  }
}
...
public:
  ...
  getHandler(const ::apache::thrift::TConnectionInfo &connInfo) {
    ...
    return new InteractiveHandler(graph, personSchema, placeSchema, orgSchema,
                                  postSchema, commentSchema, tagSchema,
                                  tagclassSchema, forumSchema);
    ...
    // start); std::cout << "Time taken: " << duration.count() << "
...
// public:
//     virtual ~InteractiveCloneFactory() {}
//     virtual InteractiveIf* getHandler(const
...
//             // std::chrono::high_resolution_clock::time_point start =
//             std::chrono::high_resolution_clock::now();
//         return new InteractiveHandler(graph, personSchema, placeSchema,
//         orgSchema, postSchema, commentSchema, tagSchema, tagclassSchema,
//         forumSchema);
//             // std::chrono::high_resolution_clock::time_point end =
//             std::chrono::high_resolution_clock::now();
...
// void ParseRequests
void ParseId(int threadId, int threadNumber) {
  // std::vector<std::thread> threads;
  // std::cout << threadNumber << std::endl;
  auto it =
      new StaticHandler(graph, personSchema, placeSchema, orgSchema, postSchema,
                        commentSchema, tagSchema, tagclassSchema, forumSchema);

  std::string filename = "/mnt/ssd/xiayanwen/test1/data/" +
                         std::to_string(threadNumber) + "trace.txt";
  ...
  while (std::getline(file, line)) {
    ...
    int firstNumber;
    if (!(iss >> firstNumber)) {
      std::cerr << "Error reading first number from line.\n";
      continue;
    }

    // std::cout << "First number: " << firstNumber << ", Remaining content: ";
    ...
    case QUERY1:
...

      {
        ...
        long long int firstLong;
        std::string str;
        long long int secondLong;
        if (!(iss >> firstLong >> str >> secondLong)) {
          // std::cerr << "Error reading long integer, string, and long integer
          // from line.\n";
          continue;
        }
        // std::cout << "First long integer: " << firstLong << ", String: " <<
        // str << ", Second long integer: " << secondLong << std::endl; break;
        idx++;
        request.personId = firstLong;
        request.firstName = str;
        request.limit = secondLong;
        it->handleQuery1(request);
      }
    ...
    case QUERY2: {
      ...
      // std::cout << "First long integer: " << firstLong << ", String: " << str
      // << ", Second long integer: " << secondLong << std::endl; break;

      request.personId = firstLong;
      request.maxDate = maxDate;
      request.limit = limit;
      it->HandleQuery2(request);
      idx++;
    } break;
    case QUERY3: {
      ...
      int32_t limit;
      if (!(iss >> personId >> countryXName >> countryYName >> startDate >>
            durationDays >> limit)) {
        continue;
      }
      idx++;
      request.personId = personId;
      request.countryXName = countryXName;
      request.countryYName = countryYName;
      request.startDate = startDate;
      request.durationDays = durationDays;
      request.limit = limit;

      it->HandleQuery3(request);
    }
    case QUERY4: {
      ...
      int32_t durationDays;
      int32_t limit;
      if (!(iss >> personId >> startDate >> durationDays >> limit)) {
        continue;
      }
      idx++;
      request.personId = personId;
...
void ParseRequests(void) {
  ...
  std::ifstream file("/mnt/ssd/xiayanwen/test1/data/113143732762368trace.txt");
...
int main(int argc, char **argv) {
  ...
  graph =
      new Graph(graphPath + "/graph.mmap", "", graphPath + "/graph.save", size);
  ...
  id_table_options.data_block_index_type =
  ...
  id_options.prefix_extractor.reset(
      rocksdb::NewFixedPrefixTransform(sizeof(uint64_t)));
  id_options.table_factory.reset(
      rocksdb::NewBlockBasedTableFactory(id_table_options));
  ...
  name_table_options.index_type =
      rocksdb::BlockBasedTableOptions::kBinarySearch;
  name_table_options.data_block_index_type =
      rocksdb::BlockBasedTableOptions::kDataBlockBinaryAndHash;
  ...
  name_options.table_factory.reset(
      rocksdb::NewBlockBasedTableFactory(name_table_options));
  ...
  personSchema =
      snb::PersonSchema(rocksdb_db, handles[0], handles[1], handles[2]);
  placeSchema =
      snb::PlaceSchema(rocksdb_db, handles[3], handles[4], handles[5]);
  orgSchema = snb::OrgSchema(rocksdb_db, handles[6], handles[7], handles[8]);
  postSchema =
      snb::MessageSchema(rocksdb_db, handles[9], handles[10], handles[11]);
  commentSchema =
      snb::MessageSchema(rocksdb_db, handles[12], handles[13], handles[14]);
  tagSchema = snb::TagSchema(rocksdb_db, handles[15], handles[16], handles[17]);
  tagclassSchema =
      snb::TagClassSchema(rocksdb_db, handles[18], handles[19], handles[20]);
  forumSchema =
      snb::ForumSchema(rocksdb_db, handles[21], handles[22], handles[23]);

  if (graph->get_max_vertex_id() == 0) {
    {
      std::vector<std::thread> pool;
      pool.emplace_back(prepareVIndex, std::ref(personSchema),
                        dataPath + snb::personPathSuffix);
      pool.emplace_back(prepareVIndex, std::ref(placeSchema),
                        dataPath + snb::placePathSuffix);
      pool.emplace_back(prepareVIndex, std::ref(orgSchema),
                        dataPath + snb::orgPathSuffix);
      pool.emplace_back(prepareVIndex, std::ref(postSchema),
                        dataPath + snb::postPathSuffix);
      pool.emplace_back(prepareVIndex, std::ref(commentSchema),
                        dataPath + snb::commentPathSuffix);
      pool.emplace_back(prepareVIndex, std::ref(tagSchema),
                        dataPath + snb::tagPathSuffix);
      pool.emplace_back(prepareVIndex, std::ref(tagclassSchema),
                        dataPath + snb::tagclassPathSuffix);
      pool.emplace_back(prepareVIndex, std::ref(forumSchema),
                        dataPath + snb::forumPathSuffix);
      for (auto &t : pool)
        t.join();
    }
...

    {
      ...
      pool.emplace_back(importForum, std::ref(forumSchema), dataPath,
                        std::ref(personSchema));

      pool.emplace_back(importRawEdge, std::ref(personSchema),
                        std::ref(tagSchema), snb::EdgeSchema::Person2Tag,
                        snb::EdgeSchema::Tag2Person,
                        dataPath + snb::personHasInterestPathSuffix);
      pool.emplace_back(importRawEdge, std::ref(postSchema),
                        std::ref(tagSchema), snb::EdgeSchema::Post2Tag,
                        snb::EdgeSchema::Tag2Post,
                        dataPath + snb::postHasTagPathSuffix);
      pool.emplace_back(importRawEdge, std::ref(commentSchema),
                        std::ref(tagSchema), snb::EdgeSchema::Comment2Tag,
                        snb::EdgeSchema::Tag2Comment,
                        dataPath + snb::commentHasTagPathSuffix);
      pool.emplace_back(importRawEdge, std::ref(forumSchema),
                        std::ref(tagSchema), snb::EdgeSchema::Forum2Tag,
                        snb::EdgeSchema::Tag2Forum,
                        dataPath + snb::forumHasTagPathSuffix);
      for (auto &t : pool)
        t.join();
    }
    {
      ...
      //            pool.emplace_back(importDataEdge, std::ref(forumSchema),
      //            std::ref(personSchema),
      //            snb::EdgeSchema::Forum2Person_member,
      //            snb::EdgeSchema::Person2Forum_member,
      //                    dataPath+snb::forumHasMemberPathSuffix,
      //                    DateTimeParser, true);
      //            pool.emplace_back(importDataEdge, std::ref(personSchema),
      //            std::ref(personSchema),  snb::EdgeSchema::Person2Person,
      //            snb::EdgeSchema::Person2Person,
      //                    dataPath+snb::personKnowsPersonPathSuffix,
      //                    DateTimeParser, true);
      pool.emplace_back(
          importDataEdge, std::ref(personSchema), std::ref(postSchema),
          snb::EdgeSchema::Person2Post_like, snb::EdgeSchema::Post2Person_like,
          dataPath + snb::personLikePostPathSuffix, DateTimeParser, true);
      pool.emplace_back(
          importDataEdge, std::ref(personSchema), std::ref(commentSchema),
          snb::EdgeSchema::Person2Comment_like,
          snb::EdgeSchema::Comment2Person_like,
          dataPath + snb::personLikeCommentPathSuffix, DateTimeParser, true);
      pool.emplace_back(
          importDataEdge, std::ref(personSchema), std::ref(orgSchema),
          snb::EdgeSchema::Person2Org_study, snb::EdgeSchema::Org2Person_study,
          dataPath + snb::personStudyAtOrgPathSuffix, YearParser, true);
      pool.emplace_back(
          importDataEdge, std::ref(personSchema), std::ref(orgSchema),
          snb::EdgeSchema::Person2Org_work, snb::EdgeSchema::Org2Person_work,
  ...
  ::server = new TThreadPoolServer(
      std::make_shared<InteractiveProcessorFactory>(
          std::make_shared<InteractiveCloneFactory>()),
  ...
  for (unsigned int i = 0; i < 8; ++i) {
    ...
    threads.emplace_back(
        [threadId, threadNumber]() { ParseId(threadId, threadNumber); });
  }
  ...
  std::cout << i << "start liveload!" << std::endl;
