...
// void HandleQuery1(std::vector<Query1Response> & _return, const Query1Request&
// request)
// {
...
//         {
//             auto nbrs = engine.get_edges(vid,
//             (label_t)snb::EdgeSchema::Person2Person); while (nbrs.valid())
//             {
//                 if(nbrs.dst_id() != root && person_hash.find(nbrs.dst_id())
//                 == person_hash.end())
//                 {
...
//                     auto firstName = std::string(person->firstName(),
//                     person->firstNameLen()); if(firstName ==
//                     request.firstName)
//                     {
//                         auto lastName = std::string(person->lastName(),
//                         person->lastNameLen());
//                         idx.push_back(std::make_tuple(k, lastName,
//                         person->id, nbrs.dst_id(), person));
//                     }
//                 }
//                 nbrs.next();
//             }
...
//         {
//             auto nbrs = engine.get_edges(vid,
//             (label_t)snb::EdgeSchema::Person2Org_work);
//             std::vector<std::tuple<uint64_t, int, snb::OrgSchema::Org*,
//             snb::PlaceSchema::Place*>> orgs; while (nbrs.valid())
//             {
//                 int year = *(int*)nbrs.edge_data().data();
